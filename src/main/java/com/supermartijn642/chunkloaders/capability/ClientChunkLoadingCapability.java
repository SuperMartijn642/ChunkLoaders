...
public class ClientChunkLoadingCapability extends ChunkLoadingCapability {
    ...
    public void readServerInfo(NBTTagCompound compound){
        ...
        NBTTagList loadedChunksPerActivePlayerTag = compound.getTagList("loadedChunksPerActivePlayer", Constants.NBT.TAG_COMPOUND);
        Streams.stream(loadedChunksPerActivePlayerTag).map(NBTTagCompound.class::cast).forEach(
            playerTag -> {
        ...
        NBTTagList loadedChunksPerInactivePlayerTag = compound.getTagList("loadedChunksPerInactivePlayer", Constants.NBT.TAG_COMPOUND);
        Streams.stream(loadedChunksPerInactivePlayerTag).map(NBTTagCompound.class::cast).forEach(
            playerTag -> {
                UUID player = playerTag.getUniqueId("player");
                Collection<ChunkPos> chunks = Streams.stream(playerTag.getTagList("chunks", Constants.NBT.TAG_COMPOUND)).map(NBTTagCompound.class::cast).map(tag -> new ChunkPos(tag.getInteger("chunkX"), tag.getInteger("chunkZ"))).collect(Collectors.toList());
                this.loadedChunksPerPlayer.putIfAbsent(player, new HashSet<>());
